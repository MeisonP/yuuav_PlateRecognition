#include "easypr/core/plate_locate.h" //类
...
/////////////////////////////////////
//车牌定位函数主体，CPlateLocate类中的plateLocate方法
int CPlateLocate::plateLocate(Mat src, vector<Mat> &resultVec, int index) {
  vector<CPlate> all_result_Plates;

  plateColorLocate(src, all_result_Plates, index);//调用同一作用域下的plateColorLocate函数
...
//此处属于函数重载（重载的条件是形参必须不一样），重载lpateLocate方法，
